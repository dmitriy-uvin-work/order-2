                        ...
                        <div class="d-flex align-items-center justify-content-between">
                            <div>
                                ...
                                <ul class="breadcrumb mb-0">
                                    ...
                                    <li class="breadcrumb-item"><a href="javascript:">Страницы</a></li>
                                </ul>
                            </div>
                            <a href="{{ route('admin.page.form') }}" class="btn btn-primary mr-0">Добавить</a>
                        </div>
                    </div>
                </div>
            </div>
                                ...
                                @if (count($data) > 0)
                                    @foreach($data as $key => $item)
                                        <tr>
                                            ...
                                            <td>
                                                {{ route('page.view', ['slug'=>$item->slug]) }}
                                            </td>
                                            <td>
                                                {!! $item->statusUI !!}
                                            </td>
                                            <td>
                                                <a href="{{ route('admin.page.form', ['id' => $item->id]) }}"
                                                   class="label theme-bg text-white f-12 mb-0">Посмотреть
                                                </a>
                                                <a
                                                    href="{{ route('admin.page.destroy', ['id' => $item->id]) }}"
